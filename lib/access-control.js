...
export async function resolveOwnershipForWrite({ actingUser, ownerIdInput, divisionIdInput }) {
	...
	if (actingUser.role === 'RECRUITER') {
		...
		if (ownerUser && ownerUser.divisionId !== actingUser.divisionId) {
			throw new AccessControlError('Assigned recruiter must belong to your division.', 403);
		}
	...
	if (!division) {
		throw new AccessControlError(
			'Division is required. Assign a recruiter in a division or provide a division value.',
			400
		);
	}

	if (ownerUser && division && ownerUser.divisionId !== division.id) {
		throw new AccessControlError('Assigned recruiter and division must match.', 400);
	}
	...
	if (division?.accessMode === 'OWNER_ONLY') {
		...
		if (!ownerId) {
			throw new AccessControlError('Assigned recruiter is required when division access mode is Owner Only.', 400);
		}
	}
